...
import { createQuestionTable, loadScoreTable, createCongratulationScore } from './GraphicUtils.js';
...
class SurvivorShooterScene extends Phaser.Scene {
  ...
  loadDungeon(){
    ...
    // Place all in the same room, set playerroom, zombieroom, zombie astar path
    ...
    function generatePlayerpos(){
      playerX=map.tileToWorldX(playerRoom.x + 1 + Math.floor(Math.random()*(playerRoom.width-2)));
      playerY=map.tileToWorldY(playerRoom.y + 1 + Math.floor(Math.random()*(playerRoom.height-2)));
    }

    // render others graphic
    generatePlayerpos();
    ...
    otherUsers.forEach((other, idx)=>{
      // generatePlayerpos();
      otherPlayers.push(new Player(SSScene, playerX+8*layer.scale, playerY+8*layer.scale, 'manBlue_machine_right_fire_', other));
    });
  ...
  create(){
    ...
    axios.post('http://localhost:8887/SurvivorScore/getDungeon', {
    })
    .then((response)=>{
      ...
      let dungeon1 = response.data;
      let clone = Object.assign(Object.create(Object.getPrototypeOf(dungeon.rooms[0])), dungeon.rooms[0]);

      // console.log(dungeon, dungeon1);

      dungeon.id = dungeon1.id;
      dungeon.width = dungeon1.width;
      dungeon.height = dungeon1.height;
      dungeon.roomGrid = dungeon1.roomGrid;
      dungeon.zombies = dungeon1.zombies;

      // update room to match server
      dungeon.rooms = [];
      dungeon1.rooms.forEach((room, idx)=>{
        let tempClone = Object.assign(Object.create(Object.getPrototypeOf(clone)), clone);
        tempClone.bottom = room.bottom;
        tempClone.centerX = room.centerX;
        tempClone.centerY = room.centerY;
        tempClone.height = room.height;
        tempClone.left = room.left;
        tempClone.right = room.right;
        tempClone.tiles = room.tiles;
        tempClone.top = room.top;
        tempClone.width = room.width;
        tempClone.x = room.x;
        tempClone.y = room.y;
        tempClone.id = room.id;
        tempClone.isContainChest = room.isContainChest;
        tempClone.isContainDeco1 = room.isContainDeco1;
        tempClone.isSPawnLocation = room.isSPawnLocation;
        tempClone.isSepcialChest = room.isSepcialChest;
      ...
      dungeon.roomGrid = dungeon1.roomGrid;
      dungeon1.roomGrid.forEach((grid, idx)=>{
      ...
      // if(debug){
      //   let debugGraphic = this.add.graphics().strokeRoundedRect(0, 0, 16, 16, 32);
